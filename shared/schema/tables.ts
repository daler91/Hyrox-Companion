...
export const serverRuntimeCache = pgTable(
...
export const analyticsResults = pgTable(
  "analytics_results",
  {
    id: varchar("id", { length: 255 })
      .primaryKey()
...
export const planDays = pgTable(
  ...
  (table) => [
    ...
    check(
      "plan_days_expected_rpe_check",
      sql`expected_rpe IS NULL OR (expected_rpe BETWEEN 1 AND 10)`,
    ),
    check(
...
export const workoutLogs = pgTable(
  "workout_logs",
  {
    ...
    planDayId: varchar("plan_day_id", { length: 255 }).references(() => planDays.id, {
      onDelete: "set null",
    }),
    planId: varchar("plan_id", { length: 255 }).references(() => trainingPlans.id, {
      onDelete: "set null",
    }),
...
export const exerciseSets = pgTable(
  "exercise_sets",
  {
    id: varchar("id", { length: 255 })
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    workoutLogId: varchar("workout_log_id", { length: 255 }).references(() => workoutLogs.id, {
      onDelete: "cascade",
    }),
    planDayId: varchar("plan_day_id", { length: 255 }).references(() => planDays.id, {
      onDelete: "cascade",
  ...
  (table) => [
    ...
    check(
      "planned_distance_non_negative_check",
      sql`planned_distance IS NULL OR planned_distance >= 0`,
    ),
    ...
    check(
      "interval_minute_non_negative_check",
      sql`interval_minute IS NULL OR interval_minute >= 0`,
    ),
...
export const workoutStructureBlocks = pgTable(
  ...
  (table) => [
    ...
    check(
      "workout_structure_block_single_owner_check",
      sql`(workout_log_id IS NULL) <> (plan_day_id IS NULL)`,
    ),
    check(
      "workout_structure_block_duration_non_negative_check",
    ...
    check(
      "workout_structure_block_work_non_negative_check",
      sql`work_seconds IS NULL OR work_seconds >= 0`,
    ),
    check(
      "workout_structure_block_rest_non_negative_check",
      sql`rest_seconds IS NULL OR rest_seconds >= 0`,
    ),
    check(
      "workout_structure_block_duration_minutes_positive_check",
      sql`duration_minutes IS NULL OR duration_minutes > 0`,
    ),
    check(
      "workout_structure_block_round_count_positive_check",
      sql`round_count IS NULL OR round_count > 0`,
    ),
    check(
      "workout_structure_block_time_cap_positive_check",
      sql`time_cap_minutes IS NULL OR time_cap_minutes > 0`,
    ),
  ],
);

export const exerciseLoadTags = pgTable(
  "exercise_load_tags",
  {
  ...
  (table) => [
    ...
    check(
      "exercise_load_tags_unilateral_non_negative_check",
      sql`${table.unilateralStability} >= 0`,
    ),
    ...
    check(
      "exercise_load_tags_tendon_modifier_positive_check",
      sql`${table.tendonLoadModifier} >= 0`,
    ),
    check(
      "exercise_load_tags_eccentric_modifier_positive_check",
      sql`${table.eccentricRiskModifier} >= 0`,
    ),
    check(
      "exercise_load_tags_running_risk_non_negative_check",
      sql`${table.highIntensityRunningRisk} >= 0`,
    ),
  ],
);

export const workoutStructureSteps = pgTable(
  "workout_structure_steps",
  {
    ...
    blockId: varchar("block_id", { length: 255 })
      .notNull()
      .references(() => workoutStructureBlocks.id, { onDelete: "cascade" }),
  ...
  (table) => [
    ...
    uniqueIndex("idx_workout_structure_steps_block_step_unique").on(
      table.blockId,
      table.stepNumber,
    ),
    uniqueIndex("idx_workout_structure_steps_block_minute_unique").on(
      table.blockId,
      table.minuteIndex,
    ),
    check("workout_structure_step_number_positive_check", sql`step_number > 0`),
    check(
      "workout_structure_step_minute_index_positive_check",
      sql`minute_index IS NULL OR minute_index > 0`,
    ),
...
export const structuredExerciseBackfillReviews = pgTable(
  ...
  (table) => [
    check(
      "structured_exercise_backfill_owner_type_check",
      sql`${table.ownerType} IN ('planDay', 'workoutLog')`,
    ),
    check(
      "structured_exercise_backfill_status_check",
      sql`${table.status} IN ('needs_manual_review', 'resolved')`,
    ),
...
export const structuredExerciseHealthCounters = pgTable(
...
export const timelineAnnotations = pgTable(
...
export const coachingMaterials = pgTable(
  ...
  (table) => [index("idx_coaching_materials_user_id").on(table.userId)],
);
...
export const documentChunks = pgTable(
  "document_chunks",
  {
    ...
    materialId: varchar("material_id", { length: 255 })
      .notNull()
      .references(() => coachingMaterials.id, { onDelete: "cascade" }),
    userId: varchar("user_id", { length: 255 })
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    content: text("content").notNull(),
...
export const aiUsageLogs = pgTable(
  ...
  (table) => [index("idx_ai_usage_logs_user_created").on(table.userId, table.createdAt)],
);
...
export const userConsents = pgTable(
  ...
  (table) => [uniqueIndex("idx_user_consents_user_type").on(table.userId, table.consentType)],
);
...
export const userTrainingStyle = pgTable(
  "user_training_style",
  {
    id: varchar("id", { length: 255 })
      .primaryKey()
  ...
  (table) => [
    ...
    check(
      "user_training_style_source_check",
      sql`source IN ('onboarding', 'settings', 'migration_default')`,
    ),
  ],
);

export const mafProfile = pgTable(
  "maf_profile",
  {
    id: varchar("id", { length: 255 })
      .primaryKey()
...
export const mafTestResults = pgTable(
  ...
  (table) => [index("idx_maf_test_results_user_created").on(table.userId, table.createdAt)],
);

export const mafWorkoutAnalysis = pgTable(
...
export const foods = pgTable(
  ...
  (table) => [
    ...
    // Functional index for case-insensitive prefix search (ILIKE 'q%'). Fuzzy/typo
    // search (pg_trgm) is enabled in migration 0074, which adds trigram GIN indexes
    // on lower(name)/lower(brand). Those are intentionally NOT declared here: the
    // `gin_trgm_ops` opclass isn't expressed via the Drizzle schema builder, and
    // declaring them would diverge from the migration snapshot and cause drift.
    // See migrations/0074_food_search_trigram.sql.
...
export const foodServings = pgTable(
  "food_servings",
  {
    id: varchar("id", { length: 255 })
      .primaryKey()
      .default(sql`gen_random_uuid()`),
    foodId: varchar("food_id", { length: 255 })
      .notNull()
...
export const foodLogEntries = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`entry_method IN ('manual','barcode','nl','photo')`,
    ),
  ],
);
...
export const mealTargets = pgTable(
  "meal_targets",
  {
    id: varchar("id", { length: 255 })
      .primaryKey()
