...
- **Missed-session recovery** - A missed session is a decision, not a red square: fold it into another day, shorten it, or let it go. Each option shows what it does to the plan before the athlete picks (the day's and week's minutes and load, the key sessions kept, and cautions such as stacking two key sessions or crowding race day), with a recommendation based on the session's tier. A let-go can be undone.
