      ...
      it("updates plan day if planDayId is present and no exercises exist", async () => {
        ...
        await waitFor(() => {
          expect(queryClientLib.apiRequest).toHaveBeenCalledWith(
            "PATCH",
            "/api/v1/plans/test-plan-id/days/pd-1",
            updates,
          );
          expect(result.current.detailEntry).toBeNull();
        });
  ...
  describe("API Callbacks and Toast notifications", () => {
    ...
    const mutationTestCases: MutationTestCase[] = [
      [
        ...
        [{ focus: "cardio", mainWorkout: "run", accessory: null, notes: null }],
        true,
      ...
      [
        ...
        { workoutLogId: "w-1", date: "2024-01-01", focus: "strength" },
        "handleSaveFromDetail",
        [{ focus: "cardio", mainWorkout: "run", accessory: null, notes: null }],
        true,
    ...
    it.each(mutationTestCases)(
      "triggers error toast on failed %s",
      async (_, mockEntry, actionName, args, needsDialog, expectedTitle) => {
