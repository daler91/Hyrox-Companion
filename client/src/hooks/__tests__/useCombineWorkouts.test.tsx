...
function createMockEntry(
  id: string,
  date: string,
  type: "planned" | "completed" = "planned",
  props: Partial<TimelineEntry> = {},
): TimelineEntry {
  return {
    id,
    date,
...
describe("useCombineWorkouts", () => {
  ...
  describe("handleCombine selections", () => {
    ...
    it.each([
      ...
      "combines sequences %s -> expects %s",
      (testEntry, isSameEntry, expectComb2, expectedToastTitle, expectComb1, expectDialog) => {
        const { result } = renderHook(() => useCombineWorkouts(), { wrapper });
        ...
        expect(mockToast).toHaveBeenCalledWith(
          expect.objectContaining({ title: "Select another workout to combine with" }),
        );
        mockToast.mockClear();

        // Step 2: secondary action (what we are actually testing parameterized)
        act(() => {
          result.current.handleCombine(testEntry);
        });

        if (expectedToastTitle) {
          expect(mockToast).toHaveBeenCalledWith(
            expect.objectContaining({ title: expectedToastTitle }),
          );
        }
  ...
  describe("handleConfirmCombine flows", () => {
    ...
    it.each([
      ...
      if (isSuccess) {
        ...
        expect(queryClientLib.apiRequest).toHaveBeenCalledWith(
          "PATCH",
          "/api/v1/plans/days/plan-1/status",
          { status: "skipped" },
        );
