...
describe("queryClient", () => {
  ...
  describe("apiRequest", () => {
    it.each([
      ...
      "should make a request with correct method %s and body provided: %s",
      async (method, data, expectedHeaders, expectedBody) => {
    ...
    it.each([
      ...
      "should throw an error %s when response is not ok",
      async (status, statusText, text, expectedError) => {
  ...
  describe("getQueryFn", () => {
    it("should return a function that calls fetch with URL joined by '/' and returns parsed JSON", async () => {
      ...
      const result = await queryFn({ queryKey } as unknown as QueryFunctionContext<
        string[],
        unknown
      >);
    ...
    it.each([
      ...
      "should handle error status %s with on401=%s correctly",
      async (on401, status, statusText, text, expected, shouldThrow) => {
        mockFetchResponse({ ok: false, status, statusText, text });
