...
describe("SessionGradeCard", () => {
  ...
  it("shows the purpose, the verdict, the evidence and the key numbers", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId("session-grade-card-w1")).toBeInTheDocument();
    });
    ...
    expect(
      screen.getByText(/From the heart-rate and pace stream · High confidence/),
    ).toBeInTheDocument();
  ...
  it("says when the grade is from averages while the stream is on its way", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/the detailed stream is on its way/)).toBeInTheDocument();
    });
  });
  ...
  it("renders nothing for a workout that is not graded", async () => {
    ...
    await waitFor(() => {
      expect(mocks.getWorkoutSessionGrade).toHaveBeenCalled();
    });
