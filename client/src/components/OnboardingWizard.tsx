...
const TITLES: Record<Step, string> = { welcome: "Welcome to HyroxTracker", units: "Set Your Preferences", goal: "What's Your Goal?", plan: "Choose Your Path", schedule: "When Do You Start?" };
const DESCS: Record<Step, string> = { welcome: "Let's get you set up in just a few steps.", units: "Choose your preferred measurement units.", goal: "This helps us tailor your experience.", plan: "How would you like to start training?", schedule: "Pick the first day of your 8-week program." };
const STEPS: Step[] = ["welcome", "units", "goal", "plan", "schedule"];
const PREV: Partial<Record<Step, Step>> = { units: "welcome", goal: "units", plan: "goal", schedule: "plan" };
...
export function OnboardingWizard({ open, onComplete }: OnboardingWizardProps) {
  ...
  const prefsMutation = useMutation({
    mutationFn: async (prefs: { weightUnit: string; distanceUnit: string }) => (await apiRequest("PATCH", "/api/preferences", prefs)).json(),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["/api/preferences"] }); queryClient.invalidateQueries({ queryKey: ["/api/auth/user"] }); },
  });

  const sampleMutation = useMutation({
    mutationFn: async () => (await apiRequest("POST", "/api/plans/sample", {})).json(),
    onSuccess: (data) => { queryClient.invalidateQueries({ queryKey: ["/api/plans"] }); setCreatedPlanId(data.id); setStep("schedule"); },
    onError: () => toast({ title: "Failed to create plan", variant: "destructive" }),
  });

  const scheduleMutation = useMutation({
    mutationFn: async ({ planId, date }: { planId: string; date: string }) => (await apiRequest("POST", `/api/plans/${planId}/schedule`, { startDate: date })).json(),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/plans"] });
      queryClient.invalidateQueries({ queryKey: ["/api/timeline"] });
      toast({ title: "Your training plan is ready!", description: "Workouts have been scheduled on your timeline." });
      markComplete(); onComplete("sample");
    },
    onError: () => toast({ title: "Failed to schedule plan", variant: "destructive" }),
  });

  const handleNext = async () => {
    if (step === "welcome") { setStep("units"); return; }
    if (step === "units") {
      try { await prefsMutation.mutateAsync({ weightUnit, distanceUnit }); } catch { toast({ title: "Could not save preferences", description: "You can update them later in settings.", variant: "destructive" }); }
      setStep("goal"); return;
    }
  ...
  return (
    <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-lg" onPointerDownOutside={(e) => e.preventDefault()}>
        ...
        <div className="flex gap-1 my-2" role="progressbar" aria-valuenow={idx + 1} aria-valuemin={1} aria-valuemax={total} aria-label={`Step ${idx + 1} of ${total}`}>
          {Array.from({ length: total }).map((_, i) => (
            <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= idx ? "bg-primary" : "bg-muted"}`} />
          ))}
        </div>
