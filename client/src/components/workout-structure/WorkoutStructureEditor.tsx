        ...
        {value.steps.map((step, idx) => (
          <div key={step.id} className="grid grid-cols-12 gap-2 items-center">
            ...
            <Select value={step.type} onValueChange={(v) => {
              ...
              steps[idx] = { ...steps[idx], type: v as StepType };
              update("steps", steps);
            }}>
              <SelectTrigger className="col-span-3"><SelectValue /></SelectTrigger>
              <SelectContent>{stepTypes.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
            </Select>
            <Input className="col-span-4" placeholder="Target" value={step.target ?? ""} onChange={(e) => {
              const steps = [...value.steps];
            ...
            <Input className="col-span-3" type="number" placeholder="Sec" value={step.durationSeconds ?? ""} onChange={(e) => {
              const steps = [...value.steps];
              steps[idx] = { ...steps[idx], durationSeconds: e.target.value ? Number(e.target.value) : undefined };
              update("steps", steps);
            }} />
            <Button type="button" variant="ghost" className="col-span-1" onClick={() => update("steps", value.steps.filter((_, i) => i !== idx))}>×</Button>
          </div>
        ))}
        <Button type="button" variant="outline" size="sm" onClick={() => update("steps", [...value.steps, { id: crypto.randomUUID(), type: "work" }])}>Add step</Button>
      </div>
