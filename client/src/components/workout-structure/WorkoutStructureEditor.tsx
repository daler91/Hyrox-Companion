...
function stepEditorHint(blockType: BlockType): string {
  ...
  if (blockType === "amrap")
    return "List each movement in the round — they repeat until the time cap.";
  ...
  return (
    <>
      <div className="mb-2 text-xs text-muted-foreground">
        {emomPreview.patternLength}-step pattern · {emomPreview.cycleCount} cycle
        {emomPreview.cycleCount === 1 ? "" : "s"}
      </div>
  ...
  return (
    <div className="space-y-3 mb-4 rounded-md border p-3">
      ...
      {value.blockType === "emom" && (
        <div className="space-y-2">
          <div className="rounded-md border border-primary/25 bg-primary/5 px-3 py-2 text-xs text-muted-foreground">
            Every Minute On the Minute: each minute starts a step. Set the total duration, then add
            the work/rest steps below.
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <div>
              ...
              <Input
                ...
                placeholder="e.g. 10"
                onChange={(e) =>
                  update("emomDurationMinutes", parseEmomDurationMinutes(e.target.value))
                }
              />
            </div>
            <div>
          ...
          <div key={step.id} className="grid grid-cols-12 gap-2 items-center rounded border p-2">
            ...
            {step.type === "work" ? (
              ...
              <div className="col-span-4 text-xs text-muted-foreground">
                {step.type === "rest" ? "Rest step" : "Transition step"}
              </div>
            )}
            <Input
      ...
      <div className="grid md:grid-cols-3 gap-2">
        <Select
          value={value.group?.kind ?? "none"}
          onValueChange={(v) =>
            update(
              "group",
              v === "none"
                ? undefined
                : { kind: v as "superset" | "circuit", restSeconds: value.group?.restSeconds },
            )
          }
        >
          <SelectTrigger>
        ...
        <Input
          ...
          onChange={(e) =>
            update(
              "group",
              value.group ? { ...value.group, name: e.target.value || undefined } : undefined,
            )
          }
        />
        <Input
          type="number"
          placeholder="Group rest (sec)"
          value={value.group?.restSeconds ?? ""}
          onChange={(e) =>
            update(
              "group",
              value.group
          ...
          <div className="space-y-2 rounded-md border p-3">
            ...
            {value.blockType === "emom" && (
              <div className="grid gap-2 md:grid-cols-3">
                <Button
                  type="button"
                  variant={
                    value.score?.type === "emom" && value.score.completed ? "default" : "outline"
                  }
                  onClick={() =>
                    updateScore(
                      mergeEmomScore(value.score, {
                        completed: !(value.score?.type === "emom" && value.score.completed),
                      }),
                    )
                  }
                >
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateScore(
                      mergeEmomScore(value.score, {
                        completedMinutes: e.target.value ? Number(e.target.value) : null,
                      }),
                    )
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    updateScore(
                      mergeEmomScore(value.score, {
                        missedReps: e.target.value ? Number(e.target.value) : null,
                      }),
                    )
                  }
                />
            ...
            {value.blockType === "amrap" && (
              <div className="grid gap-2 md:grid-cols-2">
                <Input
                  ...
                  onChange={(e) =>
                    updateScore(
                      mergeAmrapScore(value.score, { rounds: Number(e.target.value) || 0 }),
                    )
                  }
                />
                <Input
            ...
            {value.blockType === "rounds" && (
              <div className="grid gap-2 md:grid-cols-2">
                <Input
                  ...
                  onChange={(e) =>
                    updateScore(
                      mergeRoundsScore(value.score, {
                        completedRounds: Number(e.target.value) || 0,
                      }),
                    )
                  }
                />
                <Input
                  ...
                  onChange={(e) =>
                    updateScore(
                      mergeRoundsScore(value.score, {
                        elapsedSeconds: e.target.value ? Number(e.target.value) : null,
                      }),
                    )
                  }
                />
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (value.blockType === "amrap")
                  updateScore(mergeAmrapScore(value.score, { notes }));
                if (value.blockType === "rounds")
                  updateScore(mergeRoundsScore(value.score, { notes }));
              }}
            />
          </div>
        )}
