...
const UPPER_PUSH_MUSCLES = [
  "chest",
  "shoulders",
  "triceps",
] as const satisfies readonly HeatMapMuscle[];
const UPPER_PULL_MUSCLES = [
  "lats",
  "upper_back",
  "rear_delts",
  "traps",
...
const POSTERIOR_CHAIN_MUSCLES = [
  "hamstrings",
  "glutes",
  "lower_back",
] as const satisfies readonly HeatMapMuscle[];
const CORE_FRESHNESS_MUSCLES: ReadonlySet<HeatMapMuscle> = new Set<HeatMapMuscle>([
  "core",
  "obliques",
  "lower_back",
]);
...
const HEAT_TONES: readonly HeatTone[] = [
  ...
  },
  {
    ...
    zoneClass:
      "fill-orange-400 stroke-orange-800/40 dark:fill-orange-500/65 dark:stroke-orange-300/40",
...
const MUSCLE_SHAPES: readonly MuscleShape[] = [
  ...
  },
  {
    ...
    side: "front",
    muscle: "chest",
    d: "M42 80 C46 70 56 68 60 78 L60 104 C51 103 43 98 38 89 Z",
  },
  {
    kind: "path",
    side: "front",
    muscle: "chest",
    d: "M60 78 C64 68 74 70 78 80 L82 89 C77 98 69 103 60 104 Z",
  },
  ...
  { kind: "rect", side: "front", muscle: "core", x: 48, y: 107, width: 24, height: 44, rx: 8 },
  {
    ...
    side: "front",
    muscle: "obliques",
    d: "M39 105 L49 109 L48 151 L38 145 C37 130 37 116 39 105 Z",
  },
  {
    kind: "path",
    side: "front",
    muscle: "obliques",
    d: "M72 109 L81 105 C83 116 83 130 82 145 L72 151 Z",
  },
  {
    kind: "path",
    side: "front",
    muscle: "hip_flexors",
    d: "M43 156 L60 151 L77 156 L70 175 L50 175 Z",
  },
  ...
  { kind: "ellipse", side: "front", muscle: "quads", cx: 71, cy: 216, rx: 12, ry: 40 },
  {
    ...
    side: "front",
    muscle: "adductors",
    d: "M55 177 L60 178 L60 247 L51 247 C51 221 52 198 55 177 Z",
  },
  {
    kind: "path",
    side: "front",
    muscle: "adductors",
    d: "M60 178 L65 177 C68 198 69 221 69 247 L60 247 Z",
  },
  ...
  },
  {
    ...
    side: "back",
    muscle: "upper_back",
    d: "M43 79 C50 74 70 74 77 79 L73 111 C65 116 55 116 47 111 Z",
  },
  {
    kind: "path",
    side: "back",
    muscle: "lats",
    d: "M36 88 C43 96 48 110 48 132 L39 143 C34 124 32 105 36 88 Z",
  },
  {
    kind: "path",
    side: "back",
    muscle: "lats",
    d: "M84 88 C88 105 86 124 81 143 L72 132 C72 110 77 96 84 88 Z",
  },
...
// Pre-computed the filtered arrays for front and back muscle shapes outside the
// component's render cycle to eliminate redundant O(N) array allocations (.filter)
// on every render of MuscleFigure.
const MUSCLE_SHAPES_FRONT = MUSCLE_SHAPES.filter((shape) => shape.side === "front");
const MUSCLE_SHAPES_BACK = MUSCLE_SHAPES.filter((shape) => shape.side === "back");
...
function buildCoreFreshnessMetric(muscles: readonly MuscleCoverage[]): AnalysisMetric {
  const coreMuscles = muscles.filter((muscle) => CORE_FRESHNESS_MUSCLES.has(muscle.muscle));
  ...
  for (const item of trainedCore) {
    ...
    const cmp =
      ...
      (freshest.daysSince ?? Number.POSITIVE_INFINITY);
    if (cmp < 0) {
      freshest = item;
    }
  }
  const staleOrMissing = coreMuscles.filter(
    (muscle) =>
      !hasCoverageWork(muscle) || muscle.daysSince === null || muscle.daysSince > STALE_AFTER_DAYS,
  );
  ...
  const metrics: AnalysisMetric[] = [
    {
      label: "Region mix",
      value:
        totalSets > 0 && dominantRegion
          ? `${REGION_LABELS[dominantRegion[0]]} ${formatPercent(dominantRegion[1], totalSets)}`
          : "No set volume",
    ...
    },
    {
      ...
      detail:
        priorityGap?.reason ?? "Every trained muscle group is inside the 14-day freshness window.",
...
function MuscleZone({
  ...
  return (
    <g aria-label={title}>
      <title>{title}</title>
      {shape.kind === "path" ? <path d={shape.d} className={className} /> : null}
      {shape.kind === "rect" ? (
        <rect
          x={shape.x}
          y={shape.y}
  ...
  return (
    <div className="rounded-md border bg-muted/10 p-3">
      ...
      >
        ...
        <BodyGuide />
        {(side === "front" ? MUSCLE_SHAPES_FRONT : MUSCLE_SHAPES_BACK).map((shape, index) => (
          <MuscleZone
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        {formatCount(muscle.totalSets, "set", "sets")} -{" "}
        {formatCount(muscle.sessionCount, "session", "sessions")}
      </p>
    </div>
  );
}
