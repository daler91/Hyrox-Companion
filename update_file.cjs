...
const helperFunction = `  const renderPlanSelector = () => {
    if (plansLoading) {
      return (
...
let newContent = content.replace(insertionPoint, helperFunction + '\n' + insertionPoint);
newContent = newContent.replace(replaceTarget, '{renderPlanSelector()}');
