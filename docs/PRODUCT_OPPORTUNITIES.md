...
> **Eight of these have since shipped** — see [Status delta](#status-delta-since-2026-07-25)
> for which. The rows below are left as written on 2026-07-25, so their `file.ts:NNN`
> evidence points at line numbers **as of that date**; several files have since moved or
> been split (`server/storage/nutrition.ts`, for one, is now a thin facade over
> `nutritionShared`/`nutritionFoods`/`nutritionLogs`/…). Treat the filenames as the
> durable part and re-locate by symbol, not by line.
