...
## Execution Kickoff Plan (Detailed)

### Week 1 scope (start here)

1. **Create refactor safety net (Day 1)**
   - Add a small PR checklist in this document for every refactor PR:
     - API contract unchanged unless explicitly versioned
     - new/updated tests included
     - rollback plan documented
   - Baseline command set for every PR:
     - `pnpm -s check`
     - `pnpm -s lint`
     - targeted `pnpm -s test -- <scope>`

2. **Start P0 deduplication: exercise-set mutation contract (Days 2-3)**
   - Design a discriminated owner contract (`workoutLog` vs `planDay`) at use-case boundary.
   - Keep existing HTTP routes stable; migrate internals to delegate to one orchestration path.
   - Do not remove owner-specific wrappers in the first PR; mark as compatibility shims.

3. **Add parity tests before broad migration (Days 3-4)**
   - Server: parity tests for add/update/delete set behavior across owners.
   - Client: optimistic update + rollback tests for both `useWorkoutDetail` and `usePlanDayExercises` paths.

4. **Route guard consistency kickoff (Day 5)**
   - Inventory protected/mutating routes and classify current composition style.
   - Migrate one route family (recommended: workouts) to `protectedRouteBuilder` as reference implementation.

### Deliverables for the first two PRs

- **PR A — Exercise-set contract unification foundation**
  - Introduces owner-discriminated mutation contract.
  - Preserves existing endpoints and payloads.
  - Adds parity tests and rollback notes.

- **PR B — Route builder standardization (wave 1)**
  - Migrates first route family to common builder.
  - Adds middleware-order tests.
  - Documents migration template for remaining route families.

### Working agreements

- Keep PRs under ~400 net lines where practical.
- No simultaneous domain + infrastructure refactor in one PR.
- Each PR must include:
  - explicit risk level,
  - rollback steps,
  - validation command block,
  - dependency on prior PRs (if any).
