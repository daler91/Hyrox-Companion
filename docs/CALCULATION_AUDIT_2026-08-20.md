...
**Still open:** M6 (Maffetone's real categories in onboarding) and the root cause of H17/M7 —
enforcement landed as a downstream clamp, but chunks are still generated by parallel model calls with
no shared state, so the per-week ceiling remains unenforceable at generation time by construction.
Both are product decisions rather than code fixes, and are documented rather than guessed at. L4/L1
came off this list on 2026-08-26; see the Phase 1 update above.
