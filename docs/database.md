...
| `race_date` | `date` | nullable — the athlete's true race day, kept separate from `end_date` because scheduling Monday-aligns the latter |
| `retired_on` | `date` | nullable — plan lifecycle; see below |
| `generation_status` | `text` | NOT NULL, default `'ready'` |
| `generation_error` | `text` | nullable |
| `generation_started_at` | `timestamp` | nullable, default `now()` |

`retired_on` is a half-open cutoff: the plan is the athlete's active plan for
every date **before** it, and invisible from that date onwards. NULL means live.
It is how an athlete switches goals mid-block — the weeks they walk away from
stop counting towards adherence and stop being swept to `missed`, while the
weeks they actually trained keep their real status and still resolve for a past
