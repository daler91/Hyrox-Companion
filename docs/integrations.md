...
5. **Detail enrichment** (`enrichFromActivityDetail`): the list endpoint never returns `calories` (and `kilojoules` only exists for power-meter rides) or the athlete's Perceived Exertion, so the newest ≤25 imported activities (`STRAVA_DETAIL_ENRICHMENT_LIMIT`) get a best-effort `GET /api/v3/activities/{id}` detail fetch. It fills calories where the list row had none (kilojoule-derived calories stay), and, when the athlete answered Strava's "How did that feel?", copies `perceived_exertion` into `rpe` (`perceivedExertionToRpe`: same 1–10 scale, rounded, dropped if off the scale). That rating is the only RPE a sync ever writes. Failures are non-fatal; a `429` stops the enrichment loop without failing the sync. The detail is read once, at import, so a rating added on Strava afterwards is not picked up: Strava's update webhooks cover title, type and privacy changes, and the sync skips activities it already has. In practice that means ratings given on the save screen of Strava's phone app arrive, and ones added later to a watch upload do not.
...
**Heart-rate RPE suggestion.** When a logged workout has no RPE, the review sheet's picker marks the value its average heart rate suggests (dashed, with "Your heart rate suggests 7. Tap it to save, or pick your own."), and nothing is saved until the athlete taps a value. `GET /api/v1/workouts/:id` returns it as `suggestedRpe` (`server/services/rpeSuggestion.ts`): ten times the Karvonen heart-rate reserve, rounded onto 1–10 (`rpeEquivalentOfHrReserve`). That is exactly the RPE the cardio load model already scores the same as the heart rate, because both branches use `0.6 + x²·2`; `rpeSuggestion.test.ts` pins the equivalence. It is null without heart rate, without a measured max HR or an age to read it against (`hrReserveRatio` withholds rather than assume one), and for lifting, yoga and Pilates (`heartRateReflectsEffort` in `shared/deviceSportTypes.ts`, read from the recording's sport type rather than the log's title), whose average heart rate under-reads the effort. It is never written to `rpe` by the server. RPE is the only subjective signal the app has, and the coach's fatigue and undertraining flags (`computeRpeTrend`) gate what it may change in a plan, so an estimate must not reach them without the athlete confirming it. Garmin still uses the older bulk-insert path; the matcher is provider-agnostic, so wiring it is a normaliser plus the same reconciler call.
