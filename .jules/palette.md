## 2026-09-13 - Form validation errors must be wired to inputs, not just visually shown
**Learning:** `MafTestForm` and `TrainingStyleSection`'s MAF setup dialog both display validation errors with `role="alert"` (screen readers announce the error) but never connect the message to the invalid inputs via `aria-describedby` or mark them `aria-invalid`. A screen-reader user tabbing through the form after a failed submit hears nothing on the fields themselves — the error floats as a disconnected announcement. Meanwhile, `GeneratePlanScheduleStep`, `AnnotationCreateForm`, and other codebase forms already do the wiring correctly, proving the pattern is known — it was just missed on these two.
**Action:** When adding `role="alert"` to a form error message, always also (1) give it a stable `id`, (2) connect each related input with `aria-describedby={error ? id : undefined}`, and (3) mark those inputs `aria-invalid={!!error}`. The three attributes are a package deal — `role="alert"` alone only covers the moment the error appears, not subsequent field navigation.
