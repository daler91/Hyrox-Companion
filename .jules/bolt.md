...
## 2026-08-13 - Same DB row fetched twice via two different code paths in one function

**Learning:** `buildNutritionSummary` (`server/services/nutrition/nutritionSummary.ts`) fetched `storage.users.getUser(userId)` twice inside a single function: once sequentially up front (via a private `getUserTimezone` helper, needed to resolve the athlete's local "today"/window dates before the rest of the query can run) and again inside the `Promise.all` batch (to read `weightUnit`/`age`/`gender`/etc. for the load calc). This is a distinct shape from every previously-fixed pattern here — not an N+1 loop, not two independent tables that should parallelize (2026-08-11's fix), but the literal same table+row read twice by two different call sites in one function because neither call site knew the other already had it. It's easy to miss because each read looks locally justified in isolation.
**Action:** When a function needs `user` both to derive a date/timezone up front AND to read profile fields later in a `Promise.all` batch, fetch it once at the top and thread the already-resolved value into the batch (drop it from the `Promise.all` array, reference the outer `user` variable instead). Grep for `storage\.(users\.getUser|<other-single-row-getter>)` appearing more than once within one function body (not just one file) to find more of these — a second occurrence in the same function is the tell, unlike the loop-shaped N+1s which show up as a getter called *inside* a `.map`/`for`.
**Also flagged but not fixed (bigger blast radius, deferred):** `handleDailySummary` (`server/routes/nutrition/nutrition.routes.ts:259`) already has `user` in scope but its callees independently re-fetch it: `resolveDayEnergy` (`server/services/nutrition/energy.ts:37`) and, when periodized targets are on, `fetchDailyTrainingLoad`/`fetchTrainingLoadWindow` (`server/services/nutrition/dailyLoad.ts:88,152`) — up to 4 identical `getUser` calls on one Nutrition-tab page load. `resolveMealFuelTargets` in the same route file already threads `user` through as a parameter (the right pattern); `resolveDayEnergy`/`fetchTrainingLoadWindow` don't follow it yet. Left alone this run because threading `user` through changes exported function signatures shared by other callers (`fetchDailyTrainingLoad`/`fetchDailyTraining` are called from `buildDecoratedBlockPoints` and the fuelling-range service too) — worth a dedicated pass to verify every caller has (or can cheaply get) a `user` in scope before changing the signatures, rather than a same-day 50-line fix.
