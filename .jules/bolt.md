...
**Action:** Replace `Array.prototype.map` containing `.every` with a single standard `for...of` loop. Allocate a generic array string builder (`const summaries: string[] = []; summaries.push(...)`) and use a standard `for` loop with a `break;` condition instead of `.every()`. This prevents anonymous function allocations entirely and avoids unnecessary O(N) array copies, greatly reducing React hook execution time.