...
## 2026-04-27 - [Overly permissive Swagger UI CSP wildcard]
**Vulnerability:** The Swagger UI documentation route (`/api/docs`) had a loose `img-src` Content Security Policy (CSP) directive that included the entire `https:` scheme.
**Learning:** Overly permissive wildcard directives in CSP, especially broad ones like `https:`, undermine the defense-in-depth provided by a CSP by allowing resources to be loaded from any external domain, increasing the risk of data exfiltration and tracking.
**Prevention:** Always restrict CSP directives like `img-src` strictly to required origins (`'self'`, `data:`, or specific domains like `https://img.clerk.com`) instead of relying on protocol-wide wildcards.
