...
export function registerWorkoutMigrationRoutes(router: Router): void {
  protectedPost(
    router,
    "/api/v1/workouts/migration/backfill",
    { limiter: rateLimiter("migrationBackfill", 2) },
  ...
  router.get(
    "/api/v1/workouts/migration/reviews",
    isAuthenticated,
    rateLimiter("migrationReviews", 20),
    validateQuery(reviewsQuerySchema),
    asyncHandler(
      async (
        req: Request<Record<string, never>, unknown, unknown, z.infer<typeof reviewsQuerySchema>>,
        res: Response,
      ) => {
  ...
  protectedPost(
    ...
    {
      limiter: rateLimiter("migrationReviewResolve", 20),
      middleware: [validateBody(resolveSchema)],
    },
    async (
      req: Request<Record<string, never>, unknown, z.infer<typeof resolveSchema>>,
      res: Response,
    ) => {
      const userId = getUserId(req);
      const status = req.body.action === "reject" ? "needs_manual_review" : "resolved";
      const updated = await resolveBackfillReview(
        req.body.ownerType,
        req.body.ownerId,
        userId,
        status,
      ...
      if (!updated) {
        return res
          .status(404)
          .json({ error: "Migration review target not found", code: "NOT_FOUND" });
      }
      res.json({ ok: true });
    },
  );
