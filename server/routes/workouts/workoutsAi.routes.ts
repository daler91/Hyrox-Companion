...
import {
  batchReparseWorkoutsUseCase,
  reparseWorkoutFromImageUseCase,
  reparseWorkoutUseCase,
} from "../../services/parseWorkoutUseCases";
...
export function registerWorkoutAiRoutes(router: Router): void {
  ...
  protectedPost(
    router,
    "/api/v1/workouts/:id/reparse",
    {
      limiter: rateLimiter("reparse", 5),
      aiConsent: true,
      aiBudget: true,
      validation: [
        validateParams(reparseWorkoutParamsSchema),
        validateBody(reparseWorkoutRouteSchema),
      ],
    },
    async (
      req: Request<{ id: string }, unknown, z.infer<typeof reparseWorkoutRouteSchema>>,
      res: Response,
    ) => {
  ...
  protectedPost(
    ...
    async (
      req: Request<{ id: string }, unknown, z.infer<typeof parseExercisesFromImageRequestSchema>>,
      res: Response,
    ) => {
      const outcome = await reparseWorkoutFromImageUseCase({
        userId: getUserId(req),
        workoutId: req.params.id,
        image: req.body,
      });
  ...
  protectedPost(
    router,
    "/api/v1/workouts/batch-reparse",
    { limiter: rateLimiter("batchReparse", 2), aiConsent: true, aiBudget: true },
