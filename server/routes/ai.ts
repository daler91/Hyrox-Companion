...
router.post("/api/parse-exercises", isAuthenticated, rateLimiter("parse", 5), async (req: Request, res: Response) => {
  try {
...
router.post("/api/chat", isAuthenticated, rateLimiter("chat", 10), async (req: Request, res: Response) => {
  try {
...
router.post("/api/chat/stream", isAuthenticated, rateLimiter("chat", 10), async (req: Request, res: Response) => {
  try {
    const parseResult = chatRequestSchema.safeParse(req.body);
    if (!parseResult.success) {
      return res.status(400).json({ error: parseResult.error.errors[0].message });
...
router.get("/api/chat/history", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
...
router.post("/api/chat/message", isAuthenticated, async (req: Request, res: Response) => {
  try {
...
router.delete("/api/chat/history", isAuthenticated, async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
...
router.post("/api/timeline/ai-suggestions", isAuthenticated, rateLimiter("suggestions", 3), async (req: Request, res: Response) => {
  try {
    const userId = getUserId(req);
