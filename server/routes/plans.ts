...
import {
  ...
  updateTrainingPlanGoalSchema,
  workoutStatusEnum,
  dateStringSchema,
  type UpdatePlanDay,
  type PlanDay,
  type UpdateTrainingPlanGoal,
} from "@shared/schema";
...
const handlePlanDayUpdate = (
  updateFn: (
    dayId: string,
    data: UpdatePlanDay,
    userId: string,
  ) => Promise<PlanDay | null | undefined>,
) => [
  validateBody(updatePlanDaySchema),
  asyncHandler(
    async (
      req: ExpressRequest<
        { dayId: string } | { planId: string; dayId: string },
        unknown,
        UpdatePlanDay
      >,
      res: Response,
...
router.get(
  "/api/v1/plans/:id",
  isAuthenticated,
  handleGetOrDeletePlan(storage.getTrainingPlan.bind(storage)),
);

router.post(
  "/api/v1/plans/import",
  isAuthenticated,
  rateLimiter("planImport", 5),
  asyncHandler(
    async (
      req: ExpressRequest<Record<string, never>, unknown, z.infer<typeof importPlanRequestSchema>>,
      res: Response,
    ) => {
      ...
      try {
        ...
        return res
          ...
          .json({
            error:
              "Failed to parse CSV content. Please ensure it follows the expected template format.",
            code: "INVALID_CSV",
          });
      }
    },
...
router.patch(
  ...
  isAuthenticated,
  rateLimiter("planDayUpdate", 20),
  handlePlanDayUpdate((dayId, data, userId) => storage.updatePlanDay(dayId, data, userId)),
);

router.patch(
  "/api/v1/plans/days/:dayId",
  isAuthenticated,
  rateLimiter("planDayUpdate", 20),
  handlePlanDayUpdate(updatePlanDayWithCleanup),
);
...
router.patch(
  "/api/v1/plans/:id",
  isAuthenticated,
  rateLimiter("planUpdate", 20),
  validateBody(renameTrainingPlanSchema),
  asyncHandler(
    async (req: ExpressRequest<{ id: string }, unknown, { name: string }>, res: Response) => {
      const userId = getUserId(req);
...
router.patch(
  "/api/v1/plans/:id/goal",
  isAuthenticated,
  rateLimiter("planUpdate", 20),
  asyncHandler(
    async (req: ExpressRequest<{ id: string }, unknown, UpdateTrainingPlanGoal>, res: Response) => {
      const userId = getUserId(req);
      ...
      const updated = await storage.updateTrainingPlanGoal(
        req.params.id,
...
router.delete(
  "/api/v1/plans/:id",
  isAuthenticated,
  rateLimiter("planDelete", 10),
  handleGetOrDeletePlan(async (id, userId) => {
    await storage.deleteTrainingPlan(id, userId);
    return { success: true };
  }, "true"),
);

router.post(
  "/api/v1/plans/:planId/schedule",
  isAuthenticated,
  rateLimiter("planSchedule", 10),
  asyncHandler(
    async (
      req: ExpressRequest<{ planId: string }, unknown, z.infer<typeof schedulePlanRequestSchema>>,
      res: Response,
    ) => {
      ...
      if (!parseResult.success) {
        return res
          .status(400)
          .json({ error: "Invalid start date format. Must be YYYY-MM-DD", code: "BAD_REQUEST" });
      }
...
router.patch(
  "/api/v1/plans/days/:dayId/status",
  isAuthenticated,
  rateLimiter("planDayStatus", 20),
  asyncHandler(
    async (
      req: ExpressRequest<
        { dayId: string },
        unknown,
        { status?: "planned" | "completed" | "skipped"; scheduledDate?: string | null }
      >,
      res: Response,
    ) => {
      const { dayId } = req.params;
