...
16. REPEATING CIRCUITS (e.g. Hyrox): When the input is a circuit that alternates runs and stations — Hyrox is 8x 1km runs interleaved with SkiErg 1000m, Sled Push 50m, Sled Pull 50m, Burpee Broad Jumps 80m, Rowing 1000m, Farmers Carry 200m, Sandbag Lunges 100m, Wall Balls 75-100 — emit EVERY run and EVERY station as its own row in the exercises array, in order, one row per line. Do NOT collapse the repeated runs into a single row, and do NOT return the movements only inside structureBlocks with an empty exercises array. Use distance for the run/sled/carry/row/ski stations (e.g. "1000m Run" -> distance 1000, distanceUnit "m") and reps for wall balls (e.g. "100 Wall Balls" -> reps 100).
    - Example: an input listing "1000m Run", "1000m SkiErg", "50m Sled Push", … , "100 Wall Balls" on separate lines MUST produce one exercises row per line (run_1k distance 1000m, skierg distance 1000m, sled_push distance 50m, …, wall_balls reps 100) — never fewer rows than there are lines.
