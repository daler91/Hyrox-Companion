...
async function checkAndSendWeeklySummaryForUser(storage: IStorage, user: User, now: Date): Promise<boolean> {
...
async function checkAndSendMissedRemindersForUser(storage: IStorage, user: User, now: Date): Promise<boolean> {
...
export async function checkAndSendEmailsForUser(storage: IStorage, user: User): Promise<string[]> {
  ...
  if (await checkAndSendWeeklySummaryForUser(storage, user, now)) {
    sent.push("weekly_summary");
  }

  if (await checkAndSendMissedRemindersForUser(storage, user, now)) {
