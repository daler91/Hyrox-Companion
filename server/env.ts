...
process.stderr.write(
  `[env] Validating environment pid=${process.pid} at=${new Date().toISOString()}\n`,
);
...
const envSchema = z
  .object({
    ...
    AI_TEXT_OPENAI_COMPATIBLE_PROFILE: z
      .enum(["openai", "xai", "groq", "together", "openrouter", "deepseek", "custom"])
      .default("openai"),
    ...
    // Fuzzy/typo-tolerant food search via the pg_trgm trigram similarity arm of the
    // local search (migration 0074). On by default; set to "false" to fall back to
    // exact substring matching only (a runtime kill switch if a query/perf issue
  ...
  .refine(
    (data) => data.NODE_ENV !== "production" || !WEAK_ENCRYPTION_KEYS.has(data.ENCRYPTION_KEY),
    {
      ...
      message:
        "❌ FATAL: ENCRYPTION_KEY is a known weak/test placeholder; generate a real 32-byte random key",
      path: ["ENCRYPTION_KEY"],
    },
  )
  .refine(
    (data) => {
      ...
      const otherKeys = [data.ENCRYPTION_KEY, data.CSRF_SECRET].filter((k): k is string =>
        Boolean(k),
      );
    ...
    {
      message:
        "❌ FATAL: ENCRYPTION_KEY_V2 must differ from both ENCRYPTION_KEY (rotation target) and CSRF_SECRET (key separation)",
  ...
  .refine(
    (data) =>
      ...
      !data.ENCRYPTION_KEY_V2 ||
      !WEAK_ENCRYPTION_KEYS.has(data.ENCRYPTION_KEY_V2),
    {
      // W6 — the rotation key must be just as strong as the active key.
      message:
        "❌ FATAL: ENCRYPTION_KEY_V2 is a known weak/test placeholder; generate a real 32-byte random key",
      path: ["ENCRYPTION_KEY_V2"],
    },
  )
  .refine(
    ...
    (data) =>
      !(data.CLERK_PUBLISHABLE_KEY?.startsWith("pk_live_") && data.NODE_ENV !== "production"),
    {
      message:
        "❌ FATAL: pk_live_ Clerk keys detected but NODE_ENV is not 'production' — set NODE_ENV=production on this deploy",
      path: ["NODE_ENV"],
    },
  )
  .refine(
    ...
    {
      message:
        "❌ FATAL: FATSECRET_CLIENT_ID and FATSECRET_CLIENT_SECRET must be set together (or both omitted)",
