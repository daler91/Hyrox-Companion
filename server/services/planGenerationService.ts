 ...
 *
 * Nothing in code enforced it, and nothing could: chunks are generated by
 * PARALLEL model calls with no shared state, so no single call can see the
 * previous chunk's loads and the rule is unenforceable across a chunk boundary
 * by construction (audit H17). Compounded, the difference matters — 5%/week is
 * 1.48x over an 8-week build block and 3.56x over 26 weeks, turning a 100 kg
 * squat into 356 kg (audit M7).
 *
